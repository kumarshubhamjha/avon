...
use SCart\Core\Front\Models\ShopProductFeature;
use SCart\Core\Front\Models\ShopProductFaq;
...
class AdminProductController extends RootAdminController
{
    ...
    public function create()
    {
        ...
        // html add more images
        $htmlMoreFeatures = '<div class="input-group"><input type="text" id="id_sub_feature" name="sub_feature[]" value="image_value" class="form-control rounded-0 input-sm sub_image" placeholder=""  readonly="" /><input type="text" name="sub_feature_text[]" value="
                class="form-control input-sm" placeholder="Feature Text" /><span class="input-group-btn"><a data-input="id_sub_feature" data-preview="preview_sub_feature" data-type="product" class="btn btn-primary lfm"><i class="fa fa-picture-o"></i> Choose</a></span></div><div id="preview_sub_feature" class="img_holder"></div>';
        //end add more images
        ...
        $data = [
            ...
            'htmlMoreFeatures' => $htmlMoreFeatures,
    ...
    public function postCreate()
    {
        ...
        $subFeatures       = $data['sub_feature'] ?? [];
        $subFeatureHeadings       = $data['sub_feature_heading'] ?? [];
        $subFeatureSubHeadings       = $data['sub_feature_sub_heading'] ?? [];
        $subFaqQuestion       = $data['sub_faq_question'] ?? [];
        $subFaqAnswer       = $data['sub_faq_answer'] ?? [];
        ...
        //Insert sub features
        if ($subFeatures && in_array($data['kind'], [SC_PRODUCT_SINGLE, SC_PRODUCT_BUILD])) {
            $arrsubFeatures = [];
            foreach ($subFeatures as $key => $feature) {
                if ($feature) {
                    $arrsubFeatures[] = new ShopProductFeature(sc_clean(['image' => $feature, 'featureHeading' => $subFeatureHeadings[$key], 'featureSubHeading' => $subFeatureSubHeadings[$key]], [], true));
                }
            }
            $product->features()->saveMany($arrsubFeatures);
        }
        
        //Insert sub faq
        if ($subFaqQuestion && in_array($data['kind'], [SC_PRODUCT_SINGLE, SC_PRODUCT_BUILD])) {
            $arrsubFaq = [];
            foreach ($subFaqQuestion as $key => $faq) {
                if ($faq) {
                    $arrsubFaq[] = new ShopProductFeature(sc_clean(['question' => $subFaqQuestion[$key], 'answer' => $subFaqAnswer[$key]], [], true));
                }
            }
            $product->faqs()->saveMany($arrsubFaq);
        }
    ...
    public function postEdit($id)
    {
        ...
        if (in_array($product['kind'], [SC_PRODUCT_SINGLE, SC_PRODUCT_BUILD])) {
            ...
            if ($subFaqQuestion) {
                ...
                foreach ($subFaqQuestion as $key => $faq) {
                    if ($faq) {
                        $arrsubFaq[] = new ShopProductFaq(sc_clean(['question' => $subFaqQuestion[$key], 'answer' => $subFaqAnswer[$key]], [], true));
                    }
                }
                $product->faqs()->saveMany($arrsubFaq);
            }
